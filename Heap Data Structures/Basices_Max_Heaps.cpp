...
// Heaps me ele.s insert karne ke bad hm shirf "top" ko hi access kar skte hai 
// pop kar skte hai "top" ele. ko hi bas like stack data structure

// function of heaps or priority queues -: push, pop, top

// ele.s present in the heaps in a random order. 
...
// What and Why ?
// If we want a data structure in which we can always get the maximum or minimum ele.
// at any point of time then we use priority queues or heaps 


// TC-: 
// top() -: O(1) -> find the top ele. always 
// push(x) -: O(logn), n is current size ex-> hm push kiye 60 then fir se re-arrange hua ele
// and top per min or max ele. aayega, TC-O(logn)
// pop() -: O(logn)-> becoz pop hone ke bad fir se rearrange heap hota and min or max ele
// fir se top per aata hai to vh jo "searching" hoti hai uski TC=O(logn)
...
// like in sliding window -> words -: window of size k 
// for applying prefix sum -> words -: subarrays ques. 
